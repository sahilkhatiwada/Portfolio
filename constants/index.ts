// NAVIGATION
export const NAV_LINKS = [
  { href: "/", key: "home", label: "Home" },
  { href: "/", key: "about", label: "About" },
  { href: "/", key: "skills", label: "Skills" },
  { href: "/", key: "portfolio ", label: "Portfolio" },
  { href: "/", key: "testimonial", label: "Testimonial" },
  { href: "/", key: "contact", label: "Contact" },
];

// SKILLS SECTION
export const SKILLS = [
  {
    title: "frontend",
    icon: "/frontend.svg",
    description:
      "Frontend development involves crafting the visual elements of a website or application to create a user-friendly and engaging interface. Frontend developers use HTML, CSS, and JavaScript to build",
  },
  {
    title: "backend",
    icon: "/backend.svg",
    description:
      "Backend development focuses on server-side programming, databases, and server management to create the functional infrastructure that powers websites and applications",
  },
  {
    title: "fullstack ",
    icon: "/stack.svg",
    description:
      "Full-stack development encompasses both frontend and backend programming, enabling developers to create complete, end-to-end web applications, handling user interfaces,",
  },
];

// PORTFOLIO SECTION
export const PORTFOLIO = [
  {
    title: "frontend web development",
    ImgURL: "/project1.png",
    description:
      "Frontend development involves crafting the visual elements of a website or application to create",
  },
  {
    title: "backend web development",
    ImgURL: "/project2.png",
    description:
      "Backend development focuses on server-side programming, databases, and server management to create",
  },
  {
    title: "fullstack web development",
    ImgURL: "/project3.png",
    description:
      "Full-stack development encompasses both frontend and backend programming, enabling developers",
  },
];

// TESTIMONIAL SECTION
export const TESTIMONIAL = [
  {
    title: "Sophia Anderson",
    ImgURL: "/person-1.png",
    description:
      "The web development project was successfully delivered on schedule, demonstrating excellent time management and project planning.",
  },
  {
    title: "James Martinez",
    ImgURL: "/person-2.png",
    description:
      "The project boasts robust functionality with efficient back-end systems, ensuring smooth performance and scalability for future growth and development",
  },
  {
    title: "Lily Walker",
    ImgURL: "/person-3.png",
    description:
      "The website and application exhibits a high-quality user interface, intuitive navigation, and responsive design, enhancing user satisfaction.",
  },
];

// FOOTER SECTION
export const SOCIALS = [
  { href: "/", key: "github", imgURL: "/github.svg" },
  { href: "/", key: "instagram", imgURL: "/instagram.svg" },
  { href: "/", key: "twitter", imgURL: "/twitter.svg" },
  { href: "/", key: "youtube ", imgURL: "/youtube.svg" },
];

export const FOOTER_LINKS = [
  { href: "/", key: "home", label: "Home" },
  { href: "/", key: "skills", label: "Skills" },
  { href: "/", key: "portfolio ", label: "Portfolio" },
];
